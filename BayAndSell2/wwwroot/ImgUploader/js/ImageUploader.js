...
ImageUploader.prototype.scaleImage = function(img, completionCallback, orientation) {
    ...
    this.performUpload(imageData, mWidth, Math.floor(mWidth / ratio) , completionCallback);
};

ImageUploader.prototype.performUpload = function (imageData, newWidth, newHeight, completionCallback) {
    ...
    //if (this.config.debug) {
        var resizedImage = document.getElementById(this.config.OutputImg);
        var imgCarousel1_Form = document.getElementsByName(this.config.OutputImg + this.config.IMGType);
            //document.createElement('img');
        //this.config.workspace.appendChild(document.createElement('br'));
        //this.config.workspace.appendChild(resizedImage);
        imgCarousel1_Form[0].value = imageData;// скрытый ipnput с данными

    if (this.config.IMGType == "_Form") {
            $('#' + this.config.OutputImg).removeAttr("heigh");   
            $('#' + this.config.OutputImg).removeAttr("width");

            if (newWidth / newHeight <= this.config.PicWidth / this.config.PicHeight) {
                //resizedImage.setAttribute('style', 'height:' + this.config.PicHeight + 'px !important');
                $('#' + this.config.OutputImg).attr("heigh", this.config.PicHeight);
                $("[name='" + this.config.OutputImg + "_Seller_H']").val(this.config.PicHeight);
            }
            else {
                //resizedImage.setAttribute('style', 'width:' + this.config.PicWidth + 'px !important');
                $('#' + this.config.OutputImg).attr("width", this.config.PicWidth);
                $("[name='" + this.config.OutputImg + "_Seller_W']").val(this.config.PicWidth);
            }
            resizedImage.src = imageData;
            var point = document.getElementById(this.config.OutputImg + '_Point');
            point.setAttribute('style', 'border-color:red')
        }
    //}
};
...
ImageUploader.prototype.setConfig = function(customConfig) {
   ...
   // this.config.quality = 1.00;
    if (0.00 < this.config.quality && this.config.quality <= 1.00) {
        //this.config.quality = customConfig.quality;
    } else this.config.quality = 1.00;
